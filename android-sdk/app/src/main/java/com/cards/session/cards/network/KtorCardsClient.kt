...
class KtorCardsClient(
  ...
  override suspend fun paymentWithSession(
    ...
    return try {
      ...
      logger.i("Making payment session request with body: $jsonString", null)
      ...
      val response: HttpResponse = httpClient.post {
        ...
        logger.i("Full request: ${this.url} with headers: ${this.headers}", null)
      }
      logger.i("PaymentResponse: $response", null)

      if (response.status.value in 200..299) {
        val responseBody = response.body<CardsResponseDto>()
        logger.i("Payment session request successful. Response: $responseBody", null)
        responseBody
      } else {
        val errorBody = response.body<CardsErrorResponse>()
        logger.e("Payment session request failed. Error ${errorBody.error_code}: $errorBody", null)
